...
""" Local functions """
from secure_params import get_apikey, get_sru_link
from general import *
from bib_record import check_duplicate, bib_has_match, bib_to_reject, r_update_content, bib_sru
from call_api import *
from log import initiate_log, save_log
...
if __name__ == "__main__":
    ...
    ################# Read Me #########################
    # Update the name of the general params file here #
    gen_param_file = "general_params"                   
    ###################################################                   
    ...
    # Check duplicate 035 identifiers if parameter is set
    ...
    # Exit program if duplicates are found
    if gen_param['check_duplicates']['f035_empty'] != "None" :
    ...
    # Import secure parameters (API keys, SRU URL, etc.)
    secure_params_path = gen_param['secure_file_path']

    # Define API key
    api_key_nz = get_apikey(secure_params_path, "nz", env)
    api_key_iz = get_apikey(secure_params_path, inst, env)

    # Define SRU URL for bibliographic queries
    sru_link = get_sru_link(secure_params_path, env)

    # Initialize log files for import results
    ...
    ### HANDLE BIB RECORDS ###
    # Split XML file into individual records and call APIs for each record
    for record in parse_xmlFile(input_file_path).findall("record"):
        # Format XML record as string
        ...
        # Initialize list of existing holdings to avoid duplicates
        hol_alma_list = []
        hol_alma_list_id = []
        
        ## Reject record if missing holdings/items/portfolios ##
        bib_unimport = bib_to_reject(record)
        if bib_unimport[0] :
            save_log(log_bib_unimported, [bib_unimport[1], '', el035, '', '', '', r_format])
            continue
    
        ## Check for existing matches in Alma ##
        ...
        # Initialize matching status dictionary
        bib_match = {
            'exist_nz' : False,
            'exist_iz' : False,
            'sru' : [],
        ...
        # Skip record if API error occurred
        if bib_match['api_error'] :
            continue
        
        # Perform SRU search if no match and parameter is enabled
        if not bib_match['exist_nz'] and gen_param['sru_search']['bool']=='True' :
            ...
            bib_sru(record, id_list, el035, api_key_iz, sru_link, log_bib_match, env, bib_match, filter_and, filter_or)

            if bib_match['api_error'] :
                continue            

        ## If exists in NZ: update bibliographic record ##
        process_list = {}
        ...
        if bib_match['exist_nz'] :
            ...
            if update_nz_record[2] != 200 :
                ...
                continue
            process_list['nz hanging'] = 'done'

            ## If exists in IZ : update bib in IZ ##
            if bib_match['exist_iz'] :
                ...
                if get_hol_list[2] != 200 :
                    save_log(log_holding_unimported, [f"holdings check : {get_hol_list[0]}", nz_id, iz_id, '', el035, '', '', '', get_hol_list[2], get_hol_list[3]])
                    continue
        ...
        # Loop through portfolios and create if not already present
        for portfolio in record.findall("./portfolio"):
            
            # Put portfolio informations in variables
            ...
            if pf_list[1] != 200 :
                pf_to_create = False
                save_log(log_portfolio_unimported, ['no portfolios list found : ' + pf_list[0], nz_id, iz_id, '', el035, pf_coll, pf_url, pf_list[1], pf_list[2]])
                continue
            ...
            if pf_list[0] != "0" :
                ...
                for pf in pf_alma_list.findall("portfolio"):
                    ...
                    if (full_pf_alma[1] != 200) :
                        pf_to_create = False
                        save_log(log_portfolio_unimported, ['no portfolio found : ' + full_pf_alma[0], nz_id, iz_id, existing_pf_alma, el035, pf_coll, pf_url, full_pf_alma[1], pf_format])
                        break
                    ...
                    if url_pf_alma == pf_url :
                        pf_to_create = False
                        save_log(log_portfolio_unimported, ['identical portfolio found', nz_id, iz_id, existing_pf_alma, el035, pf_coll, pf_url, full_pf_alma[1], pf_format])
                        break
            ...
            if pf_to_create :
                ...
                if new_pf[1] != 200:
                    save_log(log_portfolio_unimported, ['Import error : ' + new_pf[0], nz_id, iz_id, existing_pf_alma, el035, pf_coll, pf_url, new_pf[1], pf_format])
                else :
        ...
        # Loop through holding_data fields
        for holding_data in record.findall("./holding_data"):

            # Put holding location and call number in variables
            ...
            # If call number ends with regex in json file not keep in holding
            ...
            #  Create new holdings in Alma if not already present
            if current_hol not in hol_alma_list and hol_loc != None and hol_cote != None:
                ...
                if holding[1] == 200 :
                    ...
                    save_log(log_holding_unimported, [f"holding creation error : {holding[0]}", nz_id, iz_id, '', el035, hol_loc, hol_cote_gen, hol_cote, holding[1], holding[2]])

            # Log rejected holdings with reasons
            else :
                hol_unimported_error = []
                if (current_hol in hol_alma_list) : hol_unimported_error.append("Holding already exists !")
                if (hol_loc == None) : hol_unimported_error.append("Missing location")
                if (hol_cote == None) : hol_unimported_error.append("Missing call number")
        ...
        for item in record.findall("item_data"):
            ...
            # Put item barcode, location and call number in variables 
            ...
            # For each item in the record, try to attach to matching holding
            for hol in hol_alma_list_id :
                ...
                # If no holding matches, log as unimported                                           
                if hol_iter == hol_len and barcode not in items_imported :
                    item_unimported_error = []
                    # Handles cases of missing location, missing call number, or duplicate barcodes
                    if(item_loc == None) : item_unimported_error.append("Missing location")
                    if(item_cote == None) : item_unimported_error.append("Missing call number")                                                                      
                    if(item_tuple not in hol_alma_list) : item_unimported_error.append("No holding found")
            ...
            # Log missing holdings
            if len(hol_alma_list_id) == 0 :
                save_log(log_item_unimported, ["Missing Holding", barcode, nz_id, iz_id, "", el035, item_loc, item_cote_gen, "|".join(item_cote_list), "", item_format])


        ### REMOVE UNUSED HOLDINGS ###
        for hol in hol_alma_list_id :
            hol_index = None
            
            if hol[2].startswith("22") :    # hol[2] contains hol id or error creation message 
                # Check if attached items
                item_list = get_item_list(api_key_iz, iz_id, hol[2])

                for el in holdings_logline :    # holdings_logline contains a list of lists :  nz_id, iz_id, holding[0], el035, hol_loc, hol_cote_gen, holding[1], '', '', holding[2]]
                    if (el[2] == hol[2]) :
                        hol_index = holdings_logline.index(el)
                        # Log number of items
                        holdings_logline[hol_index][6] = item_list[0]

                # If holdings have no items attached, delete them
        ...
        # Save final holdings log
        for line in holdings_logline :        
            save_log(log_holding, line)                 

# On exit, finalize and close logs
on_exit(log_files)

# End measuring program execution time
end = datetime.now()
